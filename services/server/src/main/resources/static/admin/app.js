...
async function load(){
  ...
  try{
    ...
    $('recent').innerHTML=data.recent.map(x=>`<tr><td>${new Date(x.occurred_at).toLocaleString('ko-KR')}</td><td>${x.kiosk_id}</td><td>${x.emotion_code}</td><td><span class="badge ${x.consent_status==='CONSENTED'?'yes':x.consent_status==='DECLINED'?'no':''}">${labels[x.consent_status]||x.consent_status}</span></td><td>${x.stress_score}</td><td>${x.source}</td></tr>`).join('');
